...
/** 카카오 JS SDK를 이용한 클라이언트 사이드 지오코딩 */
...
function TInput({ value, onChange, placeholder, type = 'text', className = '' }: {
  ...
  return (
    <input
      ...
      type={type}
      className={`w-full text-sm border-0 outline-none bg-transparent px-1 py-0.5 placeholder:text-gray-300 ${className}`}
    />
  )
}
      ...
      <div className="card p-5">
        ...
        {currentStep === 0 && (
          <div className="space-y-4">
              ...
              <table className="w-full border-collapse border border-gray-400 text-xs">
                <tbody>
                  ...
                  <tr>
                    ...
                    <TCell>
                      <div className="flex items-center gap-1">
                        <TInput value={form.floor} onChange={v => set('floor', v)} placeholder="1" type="number" className="w-12" />
                        <span className="text-gray-400">층 /</span>
                        <TInput value={form.total_floors} onChange={v => set('total_floors', v)} placeholder="1" type="number" className="w-12" />
                        <span className="text-gray-400">층</span>
                      </div>
                    </TCell>
                      ...
                      <div className="flex flex-wrap gap-1.5 py-0.5">
                        {PROPERTY_TYPES.map(t => (
                          <button key={t.value} type="button" onClick={() => set('property_type', t.value)}
                            className={`px-2.5 py-1 rounded text-xs border transition-colors ${form.property_type === t.value ? 'bg-brand-600 text-white border-brand-600' : 'border-gray-200 text-gray-600 hover:border-brand-300'}`}>
                            {t.label}
                          </button>
                        ))}
                      </div>
                  ...
                  <tr>
                    ...
                    <TLabel>사용승인일</TLabel>
                    <TCell>
                      <TInput value={form.approval_date} onChange={v => set('approval_date', v)} placeholder="2025.08.25" />
                    </TCell>
                    <TLabel>연 면 적(㎡)</TLabel>
                    <TCell>
                  ...
                  <tr>
                    ...
                    <TCell colSpan={5}>
                      <div className="flex flex-wrap gap-1.5 py-0.5">
                        {DIRECTIONS.map(dir => (
                          <button key={dir} type="button" onClick={() => set('direction', form.direction === dir ? '' : dir)}
                            className={`px-2.5 py-1 rounded text-xs border transition-colors ${form.direction === dir ? 'bg-brand-600 text-white border-brand-600' : 'border-gray-200 text-gray-600 hover:border-brand-300'}`}>
                            {dir}
                          </button>
                        ))}
                      </div>
            ...
            {/* 특징 */}
            <div>
              <label className="label">특장점 (복수 선택)</label>
              <div className="flex flex-wrap gap-2">
                {COMMON_FEATURES.map(f => (
                  <button key={f} type="button" onClick={() => toggleFeature(f)}
                    className={`px-3 py-1.5 rounded-full text-sm border transition-colors ${form.features.includes(f) ? 'bg-brand-600 text-white border-brand-600' : 'border-gray-200 text-gray-600 hover:border-brand-300'}`}>
                    {f}
                  </button>
                ))}
              </div>
              ...
              <div className="grid grid-cols-1 gap-3">
                <div>
                  ...
                  <div className="flex gap-2">
                    {['신축', '양호', '보통', '노후'].map(c => (
                      <button key={c} type="button" onClick={() => set('building_condition', form.building_condition === c ? '' : c)}
                        className={`px-4 py-2 rounded-lg text-sm border transition-colors ${form.building_condition === c ? 'bg-brand-600 text-white border-brand-600' : 'border-gray-200 text-gray-600 hover:border-brand-300'}`}>
                        {c}
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <textarea value={form.floor_composition} onChange={e => set('floor_composition', e.target.value)}
                    placeholder={`1층: 편의점 (임대중, 보증금 1000/월세 150)\n2층: 사무실 (공실)`}
                    rows={2} className="input resize-none text-sm" />
                </div>
                <div>
                  <label className="label">임대 현황</label>
                  <textarea value={form.rental_status} onChange={e => set('rental_status', e.target.value)}
