...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const contentType = platform === 'instagram' ? 'card_news_instagram' : 'card_news_kakao'
...
${advantages || '입지 정보 없음'}`
    ...
    if (cards.length !== 6) {
      throw new Error(`카드 수가 정확히 6장이어야 합니다 (현재: ${cards.length}장)`)
    }
    ...
    const message = e instanceof Error ? e.message : '카드뉴스 생성에 실패했습니다'
